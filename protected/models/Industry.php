...
class Industry extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			'customers' => array(self::HAS_MANY, 'Customer', 'industry_id'),
		);
	}
	...
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
         ...
         public static function loadallindustries()
	{       $industries_array = array();
        
		$industries = Industry::model()->findAll();
                
                foreach($industries as $key => $row_industry)
                    
                    {
                    
                    $industries_array[$row_industry['id']] = $row_industry['name'];
